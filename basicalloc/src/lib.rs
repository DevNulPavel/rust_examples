...
trait DbTrait: Send + Sync {
    // Одноразовый вызов
    fn get_some_data(self, key: &str) -> Result<(), ()>;
}

struct SomeDB {}

impl DbTrait for SomeDB {
    // Одноразовый вызов
    fn get_some_data(self, key: &str) -> Result<(), ()> {
        todo!()
    }
...
fn test_db_using() {
    // Стек 1
    let db: Arc<dyn DbTrait> = std::sync::Arc::new(SomeDB {});

    // Не надо ничего клонировать, тот же стек - стек 1
    db.get_some_data("123123").unwrap();
    db.get_some_data("123123").unwrap();
    db.get_some_data("123123").unwrap();

    {
        // Клонируем, чтобы работать на стеке другого потока
        let db1: Arc<dyn DbTrait> = db.clone();
        // Thread 1
        std::thread::spawn(move || {
            // Стек 2
            db1.get_some_data("asdasd").unwrap();

            // Больше не можем вызывать, self был перенесен в get_some_data выше
            // db1.get_some_data("asdasd").unwrap();
            // db1.get_some_data("asdasd").unwrap();
            // db1.get_some_data("asdasd").unwrap();
            // db1.get_some_data("asdasd").unwrap();
    ...
    {
        // Клонируем, чтобы работать на стеке другого потока
        let db2: Arc<dyn DbTrait> = db.clone();
        // Thread 1
        std::thread::spawn(move || {
            // Стек 3
            db2.get_some_data("asdasd").unwrap();
