...
#[instrument(skip(rabbit_chan, exchange_name))]
async fn create_and_bind_queue(rabbit_chan: &Channel, exchange_name: &str) -> Queue {
...
pub async fn pub_sub_example() {
    ...
    let queue1 = create_and_bind_queue(&rabbit_chan, exchange_name)
        .await;
    let queue2 = create_and_bind_queue(&rabbit_chan, exchange_name)
        .await;
    let queue3 = create_and_bind_queue(&rabbit_chan, exchange_name)
        .await;
