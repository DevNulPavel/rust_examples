...
pub async fn run(listener: TcpListener, shutdown: impl Future) {
    // Когда переданная футура 'shutdown' завершает свою работу, мы должны отправить сигнал
    // shutdown всем соединениям, для этого броадкаст канал нужен.
    // Вызов ниже игнорируем получатель из пары броадкаст-канала.
    // Когда нам нужен получатель, просто у отправителя вызываем subscribe
