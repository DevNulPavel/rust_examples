...
fn main() {
    ...
    match input.trim().parse::<usize>() {
        Ok(1) => {
            ...
            let mut stream_2 = TcpStream::connect("127.0.0.1:9191").unwrap();
            let mut buffer = [0u8;1024];
            stream_2.write_all(serde_json::to_string(&MatrixWrapper::from(&self_open_key)).unwrap().as_bytes()).unwrap();
            let size = stream_2.read(&mut buffer).unwrap();
