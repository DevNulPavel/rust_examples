...
#define u_flipped_y uniforms.u_flipped_y

// Varying defines:
#define v_texcoord output.v_texcoord
#define v_fragment_screen_coords output.v_fragment_screen_coords
...
vertex ColorInOut vertFunc(Vertex input [[stage_in]], constant ConstantBuffer& uniforms [[buffer(1)]])
{
    ...
    if (u_flipped_y) {
        v_fragment_screen_coords = vec2(output.pos.x / output.pos.w + 1.0, 1.0 - output.pos.y / output.pos.w) / 2.0;
    } else {
        v_fragment_screen_coords =  (output.pos.xy / output.pos.w + 1.0) / 2.0;
    }
    return output;
}
