...
use crate::{
    figures::{Figure, FiguresContainer},
    light::{Light, LightDistance, LightsContainer},
    material::{MaterialModificator, RefractionInfo},
    render::Ray,
...
use super::{intersection::Intersection, intersection_full::IntersectionFull};
...
impl Scene {
    ...
    pub fn trace_nearest_intersection<'a>(&'a self, ray: &Ray) -> Option<Intersection<'a>> {
        ...
        self.figures
            ...
            .min_by(|i1, i2| {
                ...
                i1.get_distance().partial_cmp(&i2.get_distance()).unwrap()
            })
    }
    ...
    pub fn trace_shadow_intersection<'a>(&'a self, ray: &Ray) -> Option<(Intersection<'a>, f32)> {
        ...
        let res = self
            ...
            .inspect(|i| {
                ...
                match i.get_object().get_material().get_modificator() {
                    MaterialModificator::Refraction(RefractionInfo {
                        transparense_level, ..
                    }) => {
        ...
        match res {
            Some(intersection) => Some((intersection, total_transparense)),
            None => None,
        }
    }

    /// Для найденного пересечения расчитываем цвет пикселя
    pub fn calculate_intersection_color(
        &self,
        ray: &Ray,
    ...
    fn calculate_light_intensivity(&self, intersection: &IntersectionFull) -> f32 {
        ...
        let light_intensivity: f32 = self
            .lights
            .iter()
            .map(|light: &dyn Light| {
                // Направление к свету
                ...
                //
                let (in_light, transparense): (bool, f32) =
                    match self.trace_shadow_intersection(&shadow_ray) {
                        Some((shadow_intersection, transparense)) => {
                            ...
                            let from_shadow_hit_to_light_dist =
                                light.distance_to_point(intersection.get_hit_point());
                            match from_shadow_hit_to_light_dist {
                                ...
                                LightDistance::Infinite => (false, transparense),
                            }
                        }
                        None => (true, 1.0),
                    };
                ...
                let light_intensity = if in_light || transparense >= 0.0 {
                ...
                let light_power = intersection.get_normal().dot(&direction_to_light)
                    * light_intensity
                    * transparense;
    ...
    fn calculate_ambient_color(&self, intersection: &IntersectionFull, cur_level: u32) -> Color {
        ...
        let ambient_color = (0..AMBIEN_ITERATIONS_COUNT).fold(Color::zero(), |acc, _| {
            ...
            let tmp_color = match ambient_intersection {
                Some(ambient_intersection) => {
                    ...
                    let ambient_color = self.calculate_intersection_color_with_level(
                        &ambient_ray,
                        &full_ambient_intersection,
                        cur_level + 1,
                    );
    ...
    fn calculate_reflection_color_without_light(
        &self,
        ray: &Ray,
        intersection: &IntersectionFull,
        cur_level: u32,
        ...
        let reflection_color = match reflection_intersection {
            Some(reflection_intersection) => {
                ...
                self.calculate_intersection_color_with_level(
                    &reflection_ray,
                    &full_reflection_intersection,
                    cur_level + 1,
                )
            }
            None => Color::zero(),
    ...
    fn calculate_reflection_color(
        ...
        let reflection_color =
            self.calculate_reflection_color_without_light(ray, intersection, cur_level)
                * reflection_level;
    ...
    fn calculate_refraction_color_without_light(
        &self,
        ray: &Ray,
        intersection: &IntersectionFull,
        index: f32,
            ...
            Some(refraction_ray) => {
                ...
                let refraction_color = match refraction_intersection {
                    Some(refraction_intersection) => {
                        ...
                        let full_refraction_intersection: IntersectionFull =
                            refraction_intersection.into();
                        self.calculate_intersection_color_with_level(
                            &refraction_ray,
                            &full_refraction_intersection,
                            cur_level + 1,
                        )
                        // }else{
                        //     Color::zero()
    ...
    fn calculate_refraction_color(
        ...
        let refraction_color = if fresnel_value < 1.0 {
            self.calculate_refraction_color_without_light(ray, intersection, index, cur_level)
        // TODO: Нужно ли +1 ?
        } else {
            Color::zero()
        };
        ...
        // Создаем луч отражения из данной точки
        let reflection_color =
            self.calculate_reflection_color_without_light(ray, intersection, cur_level); // TODO: Нужно ли +1 ?

        // Получаем смешанный цвет отражения и преломления
        let refrection_and_refraction_mix: Color = (reflection_color * fresnel_value
            + refraction_color * (1.0 - fresnel_value))
            * transparense_level
            * (*diffuse_color);
    ...
    // Для найденного пересечения расчитываем цвет пикселя
    // TODO: Use option
    fn calculate_intersection_color_with_level(
        &self,
        ray: &Ray,
        intersection: &IntersectionFull,
        cur_level: u32,
        ...
        let result_color: Color = match intersection.get_object().get_material().get_modificator() {
            ...
            MaterialModificator::Reflection(reflection_level) => {
                let reflection_color = self.calculate_reflection_color(
                    *reflection_level,
                    ray,
                    intersection,
                    diffuse_color,
            ...
            MaterialModificator::Refraction(refraction_info) => {
                let refraction_color = self.calculate_refraction_color(
                    refraction_info,
                    ray,
                    intersection,
                    diffuse_color,
            ...
            // Просто цвет
            MaterialModificator::None => *diffuse_color * light_intensivity + ambient_color,
        };
