...
impl<'r, 'c, 'd, D: DebugWrite> Executor<'r, 'c, 'd, D> {
    fn tick(&mut self) -> TickReport {
        ...
        if D::ENABLED {
            ...
            let mut chosen =
                String::with_capacity(record.chosen_prefix.len() + self.chosen.len() + 1);
            write!(
                chosen,
                "{}",
                ChosenDisplay {
            ...
            .unwrap();
            if let Some(i) = taski {
                write!(chosen, "{}", i).unwrap();
            }
            write!(self.d, "{}", chosen).unwrap();
            if chosen == self.chosen_alert {
                chosen_alert(&chosen)
            }
        }
...
impl Record<'_> {
    fn search_1<D: DebugWrite>(
        ...
        let mut coordinator = Executor::<D> {
            ...
            chosen: vec![],
            record: Some(self),
            chosen_alert: env::var("ASYNC_CV_TEST_BREAK_CHOSEN").unwrap_or(" ".into()),
        };

        loop {
            match coordinator.tick() {
                RunContinue => continue,
                r @ RunCompleteOk | r @ RunNoNew | r @ DepthLimit | r @ SearchComplete => {
                    return (r, coordinator.chosen)
                }
            }
        }
    }
