...
//     }
// };
use super::{plane::Plane, sphere::Sphere, traits::Figure};
...
impl FiguresContainer {
    ...
    pub fn iter<'a>(&'a self) -> impl Iterator<Item = &'a dyn Figure> {
        let spheres_iter = self.spheres.iter().map(|figure| {
            let light: &dyn Figure = figure;
            light
        });
        let planes_iter = self.planes.iter().map(|figure| {
            let light: &dyn Figure = figure;
            light
        });

        let final_iter = spheres_iter.chain(planes_iter);
