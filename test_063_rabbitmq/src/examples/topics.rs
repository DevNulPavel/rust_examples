...
async fn produce_data(channel: Channel, exchange_name: &str) {
    loop {
        ...
        use rand::Rng;
        let val: u8 = rand::thread_rng()
            .gen_range(1..6);
        let route_name = match val {
            1 => "logs.warning",
            2 => "logs.err",
            3 => "logs.info",
            4 => "data.type_1",
...
pub async fn topic_example() {
    ...
    rabbit_chan
        .exchange_declare(exchange_name, 
                          ExchangeKind::Topic, // Роутинг будет происходить на основании передаваемого маршрута 
                          ExchangeDeclareOptions{
                              auto_delete: true,
                              durable: true,
                              ..Default::default()
    ...
    let queue1 = create_and_bind_queue(&rabbit_chan, exchange_name, "data.*")
        .await;
    let queue2 = create_and_bind_queue(&rabbit_chan, exchange_name, "data.type_2")
        .await;
    let queue3 = create_and_bind_queue(&rabbit_chan, exchange_name, "logs.*")
        .await;
