...
fn main() {
    ...
    // Запускаем настоящую функцию main
    match real_main() {
        // Если все хорошо - выходим с кодом ошибки
        Ok(exit_code) => {
            // Вызов системного прерывания процесса
            std::process::exit(exit_code)
        },
        // Обработаем ошибку
        Err(err) => {
            // Если канал отдачи оказался закрытым - выходим без кода ошибки see PR#279
...
/// Настоящая функция main, возвращающая код ошибки
#[rustfmt::skip]
fn real_main() -> Result<i32, std::io::Error> {
    // Получаем канал вывода переменных
    let mut stdout = std::io::stdout();

    // Создаем вектор для аргументов
    ...
    // Расширяем опции из переменной окружения
    {
        let env_args = env::var("SKIM_DEFAULT_OPTIONS")
            .ok()
            .and_then(|val| {
                shlex::split(&val)
            })
    ...
    // Пропускам первый аргумент системных параметров, так как это имя программы
    ...
    // Если в параметрах есть help - выводим и выходим
    if opts.is_present("help") {
        write!(stdout, "{}", USAGE)?;
        return Ok(0);
    }

    // Если в параметрах есть version - выводим и выходим
    ...
    //------------------------------------------------------------------------------
    // Читаем непосредственно файлик историю терминала
    let fz_query_histories = opts
        .values_of("history")
        .and_then(|vals| {
            vals.last()
        });
    let cmd_query_histories = opts
        .values_of("cmd-history")
        .and_then(|vals| {
            vals.last()
        });
    ...
    let query_history = fz_query_histories
        .and_then(|filename| {
            // Для каждого файлика истории читаем строки
            read_file_lines(filename).ok() 
        })
        .unwrap_or_else(|| {
            vec![]
        });
    ...
    // Парсим опции
    let options = {
        let mut options: SkimOptions<'_> = parse_options(&opts);

        // Если есть какие-то строки в истории - сохраняем в опции
        if fz_query_histories.is_some() || cmd_query_histories.is_some() {
            ...
            // Привязывыем горячие клавиши для прыгания по истории
    ...
    //------------------------------------------------------------------------------
    // Читаем из входного канала или команду
    let stdin = std::io::stdin();
    let components_to_stop = Arc::new(AtomicUsize::new(0));
    // Определяем канал это или нет
    let rx_item = match isatty(stdin.as_raw_fd()) {
        ...
        Ok(false) | Err(nix::Error::Sys(nix::errno::Errno::EINVAL)) => {
            ...
            let pipe = CollectorInput::Pipe(Box::new(BufReader::new(stdin)));

            let (rx_item, _) = read_and_collect_from_command(components_to_stop, pipe, collector_option);
            Some(rx_item)
        }
        Ok(true) | Err(_) => {
            None
    ...
    //------------------------------------------------------------------------------
    let output_ending = if options.print0 { 
        "\0" 
    } else { 
        "\n" 
    };
...
fn read_file_lines(filename: &str) -> Result<Vec<String>, std::io::Error> {
    ...
    // Читаем содержимое
    let ret = BufReader::new(file)
        .lines()
        .collect();
    debug!("file content: {:?}", ret);
    // Отдаем результат строк в файлике
    ret
}
