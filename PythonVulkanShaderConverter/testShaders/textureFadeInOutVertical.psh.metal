...
#define u_edgebottom uniforms.u_edgebottom
#define u_edgetop uniforms.u_edgetop
...
fragment float4 fragFunc(ColorInOut input [[stage_in]], constant ConstantBuffer& uniforms [[buffer(1)]], texture2d<float> u_texture [[texture(0)]], sampler sampler_u_texture [[sampler(0)]])
{
    ...
    output = color * min(smoothstep(0.0, u_edgebottom, v_texcoord.y), 1.0 - smoothstep(u_edgetop, 1.0, v_texcoord.y));
    return output;
}
