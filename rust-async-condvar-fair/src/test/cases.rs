    ...
    D: DebugWrite,
{
    ...
    vec![Box::pin(t1), Box::pin(t2), Box::pin(tup)]
}
...
fn signal_tasks_1<D: DebugWrite>(_d: &mut D, carryon: usize) -> Vec<TestFuture> {
    ...
    (0..2)
        .map(|i| {
            (
                ...
                Box::new(move |d: &'_ mut _| signal_tasks_1(d, i)) as TasksMakerFn<_>,
            )
        })
        .collect()
}
