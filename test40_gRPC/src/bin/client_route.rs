...
async fn print_features(client: &mut RouteGuideClient<Channel>) -> Result<(), Box<dyn Error>> {
    ...
    // Вызываем получение данных у сервера в виде потока
    let mut stream = client
        .list_features(Request::new(rectangle))
        .await?
        .into_inner();

    // Обрабатываем новые данные от сервера
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Создаем клиента
    let mut client = RouteGuideClient::connect("http://127.0.0.1:10000").await?; // "http://[::1]:10000"
    ...
    // Запрашиваем у сервера список всех особенностей местности
    println!("\n*** SERVER STREAMING ***");
    print_features(&mut client).await?;

    // Закидываем на сервера наши данные потоком
    println!("\n*** CLIENT STREAMING ***");
