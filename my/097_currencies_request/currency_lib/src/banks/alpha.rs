...
pub async fn get_currencies_from_alpha(client: &Client, bank_name: &'static str) -> Result<CurrencyResult, CurrencyError> {
    // Получаем json
    ...
    Ok(CurrencyResult::new(bank_name.into(), usd_result, eur_result, time))
}